...
class Airflow(AirflowBaseView):
    ...
    def task(self, session: Session = NEW_SESSION):
        ...
        if not dag or task_id not in dag.task_ids:
            flash(f"任务 [{dag_id}.{task_id}] 目前似乎不存在", "error")
            return redirect(url_for("Airflow.index"))
        ...
        no_failed_deps_result = [
            (
                "未知, 所有依赖关系都已满足，但任务实例尚未运行。在大多数情况下，这通常意味着任务很可能很快会被调度，除非：<br>\n- 调度器宕机或负载过重<br>\n{}\n" \
                "<br>\n如果此任务实例未能很快启动，请联系您的 Airflow 管理员寻求帮助。".format(
                    "- 此任务实例已经运行并且其状态已被手动更改（例如，在 UI 中被清除）<br>"
    ...
    def trigger(self, dag_id: str, session: Session = NEW_SESSION):
        ...
        flash(f"触发 {dag_id} 成功， 运行实例： {dag_run.run_id}。")
        if "/grid?" in origin:
    ...
    def conf(self):
        ...
        if expose_config in {"non-sensitive-only", "true", "t", "1"}:
            ...
            return self.render_template(
                ...
                title=title,
                hide_config_msg=(
                    "处于安全考虑，系统配置信息不可见，如果需要查看，请联系管理员修改安全配置。"
                ),
            )
...
class PoolModelView(AirflowModelView):
    ...
    label_columns = {
        ...
        "description":"描述",
        "slots":"资源槽",
        "running_slots":"当前正在占用的插槽",
        "queued_slots":"已排队任务占用的插槽",
        "scheduled_slots":"已调度任务占用的插槽",
        "deferred_slots":"已延迟任务占用的插槽"
    }
    ...
    include_deferred_field = BooleanField(
        ...
        label="计算可用插槽是否包含延迟任务",
        description="勾选此项以在计算可用池插槽时包括延迟任务。",
    )
...
class LogModelView(AirflowModelView):
    ...
    label_columns = {
        "execution_date": "逻辑执行日期",
        "owner": "所有者",
        "owner_display_name": "所有者名称",
        "dttm":"日期",
        "dag_id":"流程",
        "task_id":"任务",
        "run_id":"运行实例",
...
class TaskInstanceModelView(AirflowModelView):
    ...
    label_columns = {
        ...
        "queue":"队列",
        "queued_dttm":"队列时间",
        "prev_attempted_tries":"重试次数",
        "pool":"资源池",
        "queued_by_job_id":"被放入队列的作业ID",
        "external_executor_id":"外部执行器",
        "log_url":"日志",
        "execution_date":"逻辑时间"
    }
