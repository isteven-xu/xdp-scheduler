...
const DagRunTooltip = ({ dagRun }: Props) => {
  ...
  return (
    <Box py="2px">
      <Text>状态: {dagRun.state || "no status"}</Text>
      <Text whiteSpace="nowrap">
        运行时间: <Time dateTime={getDagRunLabel({ dagRun, ordering })} />
      </Text>
      <Text>运行时长: {formatDuration(dagRun.duration)}</Text>
      <Text>实例类型: {dagRun.runType}</Text>
      {dagRun.note && <Text>备注</Text>}
    </Box>
  );
};
